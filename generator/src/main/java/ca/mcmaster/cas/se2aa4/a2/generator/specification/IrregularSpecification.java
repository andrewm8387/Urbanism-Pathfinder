...
public class IrregularSpecification implements Buildable {
    ...
    private final int width, height, nbTiles, relax, numberOfCities;
    public IrregularSpecification(int width, int height, int nbTiles, int relax, int numberOfCities) {
     ...
     IrregularSpecification(Map<String, String> options){
        this(Integer.parseInt(options.getOrDefault(Configuration.WIDTH, "1000")),
                ...
                , Integer.parseInt(options.getOrDefault(Configuration.CITIES, "100")));
    }
    ...
    public Mesh build() {
        ...
        Mesh result = new Mesh(this.width, this.height, this.numberOfCities);
