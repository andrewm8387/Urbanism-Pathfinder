...
public class Exporter {
    ...
    private Map<Vertex, Integer>  registerVertices(Mesh mesh, Structs.Mesh.Builder result) {
        ...
        for(Vertex key: vertices.keySet()){
            int nextVertexIndex = vertices.get(key.getNextVertex());
            Structs.Property citySize = Structs.Property.newBuilder().setKey("citySize").setValue(key.getCitySize().toString()).build();
            Structs.Property nextVertex = Structs.Property.newBuilder().setKey("nextVertex").setValue(String.valueOf(nextVertexIndex)).build();
            Structs.Vertex v = Structs.Vertex.newBuilder().setX(key.x()).setY(key.y()).addProperties(0, citySize).addProperties(1, nextVertex).build();
